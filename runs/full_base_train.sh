...
# Train a d24 base model in two stages:
# 1. gigaverbo-v2
# 2. all gigaverbo-v2-synth training shards, resumed from stage 1
#
# Example:
# WANDB_RUN=full screen -L -Logfile runs/full_base_train.log \
#   -S full_base_train bash runs/full_base_train.sh
...
# Approximately 9.076B tokens, matching all 37 synthetic training shards.
# The 38th synthetic shard is reserved for validation.
SYNTH_TARGET_PARAM_DATA_RATIO="${SYNTH_TARGET_PARAM_DATA_RATIO:-11.634342}"
SYNTH_WARMDOWN_RATIO="${SYNTH_WARMDOWN_RATIO:-1.0}"
WANDB_RUN="${WANDB_RUN:-full}"
...
# Download enough base data to start, then fetch the remaining base and synth
# shards while stage 1 trains.
...
run_cmd torchrun --standalone --nproc_per_node="$NPROC_PER_NODE" -m scripts.base_train -- \
    ...
    --run="$WANDB_RUN"
wait "$BASE_DOWNLOAD_PID"

run_cmd torchrun --standalone --nproc_per_node="$NPROC_PER_NODE" -m scripts.base_eval -- \
    --device-batch-size="$DEVICE_BATCH_SIZE"

# -----------------------------------------------------------------------------
# Base model pretraining, stage 2 on all gigaverbo-v2-synth shards

# The download has overlapped stage 1 and its evaluation. Wait for any remaining
# shards and validate the dataset before the loader snapshots its file list.
wait "$SYNTH_DOWNLOAD_PID"
